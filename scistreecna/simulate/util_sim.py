...
def add_copy_number_noise(reads, noise_prob):
    ...
    noisy_matrix[:, :, 2][is_two_and_flip] = noisy_matrix[:, :, 2][is_two_and_flip] + (1 if np.random.rand() > .5 else -1)
...
def add_copy_number_noise2(reads, noise_prob):
    ...
    copy_numbers = noisy_matrix[:, :, 2]
    flip_mask = np.random.rand(*copy_numbers.shape) < noise_prob
    noisy_matrix[:, :, 2][flip_mask] = noisy_matrix[:, :, 2][flip_mask] + (1 if np.random.rand() > .5 else -1)
    return noisy_matrix, flip_mask
...
def add_copy_number_noise3(reads, noise_prob):
    ...
    copy_numbers = noisy_matrix[:, :, 2]
    flip_mask = np.random.rand(*copy_numbers.shape) < noise_prob
    noisy_matrix[:, :, 2][flip_mask] = noisy_matrix[:, :, 2][flip_mask] + np.random.choice([-1, 1], size=flip_mask.sum())
    return noisy_matrix, flip_mask
