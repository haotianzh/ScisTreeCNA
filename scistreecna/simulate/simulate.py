...
def generate_cell_lineage_tree(n_cells=50, scale=1):
    ...
    tree.root.branch = 1
    util.apply_attr_on_tree(tree, 'branch', lambda x: x.branch * scale) # branch is a way too large at default.
    return tree
...
def generate_sample_clt(tree, 
                        n_site, 
                        n_vaiant_per_site=1, 
                        error=0.01, dropout=0.2, 
    ...
    res = sp.run(f'{scism_path} {tmpfile} {n_site} {n_vaiant_per_site} {error} {dropout} {doublet} {rate_cn_gain} {rate_cn_loss} {recurrent} 0 {dropout_cell_variance} {coverage_mean} {coverage_std} {random_seed} {int(beta_binomial)}', shell=True, stdout=sp.PIPE, text=True)
    for line in res.stdout.splitlines():
        if 'Number of sites' in line:
            nsite = int(line.strip().split(':')[1])
        if 'Read Count (0,1)' in line:
            reads_wild.append(int(line.strip().split('=')[1].strip()[1:-1].split(',')[0]))
            reads_mut.append(int(line.strip().split('=')[1].strip()[1:-1].split(',')[1]))
...
def generate_sample_clone(tree, 
                            n_site, 
                            n_vaiant_per_site=1, 
                            error=0.01, dropout=0.2, 
                            dropout_cell_variance=0, 
    ...
    for cid in range(max(cluster_ids)+1):
        cn[:, cluster_ids == cid] = np.mean(cn[:, cluster_ids == cid], axis=1, keepdims=True)
    # cn[cn == 0] = 1
    data = np.concatenate([reads_wild, reads_mut, cn], axis=-1)
    return data, tg
...
def simulate_data(n_cells, n_sites, mode='clt', n_clusters=4, cn_noise=0.05, missing=0):
    ...
    if mode == 'clone':
        ...
        tg = util.ggeno_to_bgeno(tg)
    if cn_noise > 0:
        reads, masks = util_sim.add_copy_number_noise2(reads, noise_prob=cn_noise) # add noise to copy numbers
    if missing > 0:
