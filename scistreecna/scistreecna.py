...
class ScisTreeCNA:
    ...
    def init_prob_leaves_gpu(self, reads, ado=0.1, seqerr=0.001, cnerr=0.2, af=None):
        ...
        #     np.int32(self.CN_MAX), np.int32(self.CN_MIN), np.int32(N)
        # ))
        util.compute_genotype_log_probs_cn_noise()((blocks_per_grid,), (threads_per_block,), (
            ref.ravel(), alt.ravel(), cn.ravel(), afs, probs.ravel(),
            np.float32(ado), np.float32(seqerr), np.float32(cnerr),
            np.int32(ncell), np.int32(nsite),
            np.int32(self.CN_MAX), np.int32(self.CN_MIN), np.int32(N)
        ))
        probs = cp.ascontiguousarray(cp.transpose(probs.reshape((nsite, ncell, N)), (1, 0, 2)))
    ...
    def calculate_U_batch(self, probs, trees, batch_size=512):
        ...
        for nodes in loader(order='up'):
            ...
            if not INIT:
                ...
                util.batch_matadd_cuda()(grid_size, block_size, (ptr_c0_u_, ptr_c1_u_, ptr_u, len(nodes), h, w))
            # calculate U_ and _U          
            util.batch_log_matmul_cuda()(grid_size, block_size, (ptr_u, self.tran_prob_mutation_free, ptr_u_, len(nodes), h, w))
            util.batch_log_matmul_cuda()(grid_size, block_size, (ptr_u, self.tran_prob_mutation, ptr__u, len(nodes), h, w)) 
    ...
    def calculate_Q_batch(self, probs, trees, batch_size=512):
        ...
        for nodes in loader(order='down'):
            ...
            if not INIT:
                ...
                util.batch_matadd_cuda()(grid_size, block_size, (ptr_par_q, ptr_u, ptr_q_, len(nodes), h, w))
                util.batch_log_matmul_cuda()(grid_size, block_size, (ptr_q_, tran_prob_mutation_free_t, ptr_q, len(nodes), h, w))
    ...
    def marginal_evaluate_dp_batch(self, probs, trees, batch_size=512):
        ...
        likelihoods = cp.log(cp.zeros((len(loader)-len(trees), h), dtype=cp.float32)) # ((n-1)) * n_tree (no mut on root) n: #all nodes 
        ...
        for nodes in loader(order='all'):
            ...
            util.batch_log_3vecdot_cuda()(grid_size, block_size, (ptr__u, ptr_u_, ptr_q, res, len(ptr_u), h, w))
            tids += tid
            idx += len(ptr_u)
            
        likelihoods = likelihoods.reshape(len(trees), -1, h)
        likelihoods = cp.concatenate([likelihoods, likelihoods_root.reshape(len(trees), -1, h)], axis=1)
    ...
    def calcualte_U(self, tree, probs):
        ...
        for node in self.traversor(tree):
            if node.is_leaf():
                ...
                util.log_matmul_cuda()(grid_size, block_size, (node.U, self.tran_prob_mutation_free, node.U_, h, w))
                util.log_matmul_cuda()(grid_size, block_size, (node.U, self.tran_prob_mutation, node._U, h, w))
                continue
            components = []
    ...
    def marginal_evaluate_dp(self, probs, tree):
        ...
        for node in self.traversor(tree):
            ...
            for sib in node.get_siblings():
                res += sib.U_
            likelihood = util.log_mat_vec_mul(node.parent.Q, res.reshape(nsite, 1, self.N))
    ...
    def nni_search_sinlge_round_batch(self, probs, tree, tree_batch_size=64, node_batch_size=32):
        """
            NNI neighbor
        """
        ...
        for bi, trees in enumerate(loader()):
            # print(f'#batch: {bi}')
            likelihoods = self.marginal_evaluate_dp_batch(probs, [_.copy() for _ in trees], batch_size=node_batch_size)
    ...
    def local_search_batch(self, probs, tree, ground_truth=None, tree_batch_size=64, node_batch_size=32):
        # tree = self.initial_tree(probs)
        L = -np.inf
        iters = 0
        while True:
            better_tree, likelihood = self.nni_search_sinlge_round_batch(probs, tree, tree_batch_size=tree_batch_size, node_batch_size=node_batch_size)    
            if likelihood <= L:
                print('Converge, Stop!')
                break
            else:
                ...
                if ground_truth:
                    print(f'Tree accuracy: {util.tree_accuracy(ground_truth, tree):.4f}')
                iters += 1
        return tree, L
                ...
                for node2 in self.traversor(t):
                    if node2.is_leaf():
                        ...
                        for child in node2.get_children():
                            ...
                            val_, arg_ = util.log_mat_vec_max(tran_prob_mut_free_broadcast, state)
    ...
    def viterbi_decoding(self, probs, tree, sites):
        ...
        decoded_trees = []
        max_L, likelihoods, trees = self.maximal_evaluate(probs, tree, return_trees=True)
        for site in sites:
            L = likelihoods[:, site]
...
def infer(reads, cn_min=1, cn_max=4, cn_noise=0.05, tree_batch_size=64, node_batch_size=64, true_tree=None):
    ...
    s = ScisTreeCNA(CN_MAX=cn_max, CN_MIN=cn_min, LAMBDA_C=cn_avg, LAMBDA_S=1, LAMBDA_T=2*n_cells-1, verbose=False)
    probs = s.init_prob_leaves_gpu(reads, cnerr=cn_noise, af=0.5)
    tree, likelihood = s.local_search_batch(probs, start_tree, tree_batch_size=tree_batch_size, node_batch_size=node_batch_size, ground_truth=true_tree)
